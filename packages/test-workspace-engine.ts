...
} from "./workspace/workspace-errors";
...
function assertThrows(fn: () => any, errorClass: any, message: string): void {
    try {
        fn();
        failed++;
...
async function assertAsyncThrows(fn: () => Promise<any>, errorClass: any, message: string): Promise<void> {
    try {
        await fn();
        failed++;
        errors.push(`FAIL (no throw): ${message}`);
...
async function testTransactionLifecycle(): Promise<void> {
...
async function testRollbackOnError(): Promise<void> {
...
async function testMultipleOperations(): Promise<void> {
...
async function testRenameFile(): Promise<void> {
    console.log("\n── 6. File Rename ────────────────────────────────────────────");
    const root = makeTempWorkspace();
    try {
        const engine = new WorkspaceEngine({ workspaceRoot: root });
...
async function testPatchEngine(): Promise<void> {
    console.log("\n── 7. Patch Engine ───────────────────────────────────────────");
...
async function testLockManager(): Promise<void> {
    console.log("\n── 8. Lock Manager ───────────────────────────────────────────");
    ...
    assertThrows(
        () => locks.acquire("/a/b/c.ts", "tx-002"),
        WorkspaceLockError,
        "Lock contention throws WorkspaceLockError"
    );
...
async function testLockContention(): Promise<void> {
    ...
    try {
        ...
        assertThrows(
            ...
            WorkspaceLockError,
            "Second transaction cannot stage a locked path"
        );
...
async function testJournal(): Promise<void> {
    ...
    try {
        ...
        const beginEntry = entries.find((e: any) => e.action === "begin");
        const writeEntry = entries.find((e: any) => e.action === "write");
        const commitEntry = entries.find((e: any) => e.action === "commit");
...
async function testValidateFile(): Promise<void> {
...
async function testApplyArtifacts(): Promise<void> {
...
async function testApplyArtifactsEmpty(): Promise<void> {
...
async function testConcurrentTransactionIsolation(): Promise<void> {
...
async function testPartialRollbackOnMultiOpError(): Promise<void> {
...
async function testUseCommittedTransactionIDError(): Promise<void> {
    ...
    try {
        ...
        assertThrows(
            () => engine.stage(tx.id, { kind: "WriteFile", path: "y.ts", content: "y" }),
            WorkspaceTransactionError,
            "Staging on committed transaction throws WorkspaceTransactionError"
        );
    } finally {
        cleanupWorkspace(root);
    }
...
async function testRegressionAgentRuntime(): Promise<void> {
