...
export class MockSDKProvider extends BaseSDKProvider {
    readonly id = "mock-sdk-provider";
    readonly name = "Mock SDK Provider";
    ...
    async execute(
        ...
        const artifact = {
            ...
            taskId: task.id,
            type: this.artifactType(task.type) as any,
            path: task.file,
    ...
    private artifactType(capability: AgentCapability): string {
        switch (capability) {
    ...
    private generateContent(task: RuntimeTask): string {
        return [
            `// Task: ${task.title}`,
            `// Type: ${task.type}`,
            `// Provider: ${this.id}`,
