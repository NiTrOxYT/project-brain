...
export class ProviderPolicyInstaller {
    static getInstructionsPath(providerId: string): string {
    ...
    static isPolicyInstalled(providerId: string): boolean {
    ...
    static installPolicy(providerId: string): { success: boolean; error?: string } {
        const filepath = this.getInstructionsPath(providerId);
        if (!filepath) {
    ...
    static removePolicy(providerId: string): { success: boolean } {
        const filepath = this.getInstructionsPath(providerId);
        if (!filepath || !fs.existsSync(filepath)) {
            return { success: true };
        }
