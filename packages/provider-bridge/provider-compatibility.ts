...
export class ProviderCompatibilityRegistry {
    ...
    ): { supported: boolean; isNewer: boolean; warning?: string; error?: string } {
    ...
    static getRecommendation(compatibility: ProviderCompatibility, installedVersion: string): string {
        const compMin = compareVersions(installedVersion, compatibility.minimumVersion);
        if (compMin < 0) {
