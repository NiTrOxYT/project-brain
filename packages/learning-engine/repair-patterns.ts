...
export class RepairPatternsLearner {
    learn(experiences: LearningExperience[], existing: RepairPattern[]): RepairPattern[] {
        const patternsMap = new Map<string, RepairPattern>();
        ...
        for (const exp of experiences) {
            ...
            for (const err of exp.errors) {
                ...
                if (!p.evidence.some(e => e.executionId === exp.id)) {
                    ...
                    p.averageDurationMs = Math.round(
                        (p.averageDurationMs * (n - 1) + exp.durationMs) / n
                    );
    ...
    private parseError(err: string): { errorType: string; pattern: string; recommendedFix: string } | null {
