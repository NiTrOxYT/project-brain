...
import { handleError, EXIT_VALIDATION, CliError } from "./utils/errors.js";
...
function parseArgs(argv: string[]): ParsedArgs {
    ...
    while (i < argv.length) {
        ...
        if (arg.startsWith("--")) {
            ...
            for (const ch of arg.slice(1)) {
                if (ch === "v") flags.set("verbose", true);
                else if (ch === "q") flags.set("quiet", true);
                else if (ch === "j") flags.set("json", true);
                else if (ch === "h") flags.set("help", true);
            }
        } else {
    ...
    if (positionals.length > 1) subcommand = positionals[1];

    return { command, subcommand, flags, positionals };
}

function flag(flags: Map<string, string | boolean>, key: string): boolean {
...
function flagNum(flags: Map<string, string | boolean>, key: string): number | undefined {
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
    ...
    if (flag(flags, "version")) {
        ...
        else      process.stdout.write(`brain ${VERSION}\n`);
        return;
    }
    ...
    const opts: GlobalOptions = { workspace, project, json, verbose, quiet };
    ...
    try {
        switch (command) {
            ...
            case "query": {
                ...
                await runQuery(opts, {
                    query:  flagStr(flags, "query"),
                    format: (flagStr(flags, "format") as "json" | "text" | undefined),
                });
                break;
            }
            ...
            case "workflow": {
                if (!subcommand) {
                    process.stdout.write(
                        "Usage: brain workflow <run|resume|cancel|status|history|report|diagnostics> [options]\n"
                    );
                    process.exit(EXIT_VALIDATION);
                }
                const { runWorkflow } = await import("./commands/workflow.js");
                await runWorkflow(opts, subcommand as any, {
            ...
            case "runtime": {
                ...
                await runRuntime(opts, subcommand as any, {
            ...
            case "shared-memory": {
                if (!subcommand) {
                    process.stdout.write(
                        "Usage: brain shared-memory <status|agents|tasks|conflicts|consensus|snapshot|restore|statistics|diagnostics>\n"
                    );
                    process.exit(EXIT_VALIDATION);
                }
                const { runSharedMemory } = await import("./commands/shared-memory.js");
                await runSharedMemory(opts, subcommand as any, {
            ...
            case "context": {
                ...
                await runContext(opts, subcommand as any, {
                    to:   flagStr(flags, "to"),
            ...
            case "workspace": {
                ...
                await runWorkspaceCmd(opts, subcommand as any, {
                    tx: flagStr(flags, "tx"),
                });
                break;
            }
            ...
            case "learning": {
                ...
                await runLearning(opts, subcommand as any, {
            ...
            case "config": {
                ...
                await runConfig(opts, subcommand as any, {
