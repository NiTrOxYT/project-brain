    ...
    try {
        switch (sub) {
            case "latest": {
                ...
                spinner.stop();
                if (!snap) {
                    if (opts.json) printJson({ ok: true, snapshot: null });
                    else logger.log(gray("No snapshot found. Run: brain compile"));
                    break;
                }
                if (opts.json) {
            ...
            case "validate": {
                const snap = await svc.latestSnapshot();
                if (!snap) throw new ValidationError("No snapshot to validate");
            ...
            case "compact": {
                ...
                spinner.stop();
                if (opts.json) printJson({ ok: true, status: "compacted", removed });
                else logger.log(`Context storage compacted. Removed ${removed} old snapshot(s).`);
                break;
            }

            case "rollback": {
                const targetId = cmdOpts["to"] as string | undefined;
                if (!targetId) throw new ValidationError("--to <snapshot-id> is required");
            ...
            case "delta": {
                const fromId = cmdOpts["from"] as string | undefined;
                ...
                const loadSnap = (id: string) => {
                    const candidates = fs.existsSync(snapDir)
                        ...
                        : [];
                    if (candidates.length === 0) throw new ValidationError(`Snapshot not found: ${id}`);
