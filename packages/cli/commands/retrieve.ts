...
export async function runRetrieve(opts: GlobalOptions, cmdOpts: RetrieveOptions): Promise<void> {
    ...
    try {
        ...
        const totalTokens = result.retrievalPackage.sections.reduce(
            (acc, s) => acc + (s.estimatedTokens ?? 0), 0
        );
