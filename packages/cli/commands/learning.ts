...
type LearningSubcmd = "learn" | "recommend" | "statistics";
...
export async function runLearning(
    ...
    try {
        switch (sub) {
            case "learn": {
                const eventPath = cmdOpts["event"] as string | undefined;
                if (!eventPath) throw new ValidationError("--event <path-to-event-json> is required");
            ...
            case "recommend": {
                const query = cmdOpts["query"] as string | undefined;
