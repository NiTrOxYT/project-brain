...
export async function runWorkspaceCmd(
    opts: GlobalOptions,
    sub: WorkspaceSubcmd,
        ...
        switch (sub) {
            case "status": {
                ...
                if (opts.json) {
                    ...
                    logger.log(renderKeyValue(Object.entries(status as unknown as Record<string, unknown>).map(([k, v]) => [k, String(v)])));
                }
                break;
            }

            case "transactions": {
                const stagedMap = (engine as any).staged ?? new Map();
            ...
            case "locks": {
                const locks = (engine as any).locks?.activeLocks() ?? [];
                spinner.stop();
                if (opts.json) {
                    ...
                    if (locks.length === 0) {
                        ...
                        logger.log(renderTable(
                            ...
                            locks.map((l: any) => ({ path: l.path ?? "", txId: l.transactionId ?? "", mode: l.mode ?? "" }))
                        ));
                    }
                }
                break;
            ...
            case "rollback": {
                const txId = cmdOpts["tx"] as string | undefined;
                if (!txId) throw new ValidationError("--tx <transaction-id> is required");
