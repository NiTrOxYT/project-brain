...
export class SnapshotNormalizer {
    ...
    normalizePath(filePath: string, base: string): string {
        const rel = path.relative(base, filePath);
        return rel.split(path.sep).join("/");
    }

    /** Normalize and sort SnapshotFile array. */
    normalizeFiles(files: SnapshotFile[]): SnapshotFile[] {
    ...
    normalizeSymbols(symbols: SnapshotSymbol[]): SnapshotSymbol[] {
        const seen = new Set<string>();
    ...
    normalizeDependencies(deps: SnapshotDependency[]): SnapshotDependency[] {
        const seen = new Set<string>();
    ...
    normalizeRelationships(rels: SnapshotRelationship[]): SnapshotRelationship[] {
        const seen = new Set<string>();
        ...
        return unique.sort((a, b) => {
            ...
            if (predComp !== 0) return predComp;
            return a.object.localeCompare(b.object);
        });
    }

    /** Normalize graph nodes — sort by id. */
    normalizeNodes(nodes: SnapshotGraphNode[]): SnapshotGraphNode[] {
        const seen = new Set<string>();
    ...
    normalizeEdges(edges: SnapshotGraphEdge[]): SnapshotGraphEdge[] {
        const seen = new Set<string>();
    ...
    normalizeArchitecture(entries: SnapshotArchitectureEntry[]): SnapshotArchitectureEntry[] {
        const seen = new Set<string>();
    ...
    normalizeEvolution(entries: SnapshotEvolutionEntry[]): SnapshotEvolutionEntry[] {
        const seen = new Set<string>();
        const unique = entries.filter(e => {
    ...
    normalizeLearning(entries: SnapshotLearningEntry[]): SnapshotLearningEntry[] {
        const seen = new Set<string>();
        const unique = entries.filter(e => {
    ...
    topologicalSort(
        ...
        for (const edge of edges) {
            if (!inDegree.has(edge.toId) || !inDegree.has(edge.fromId)) continue;
        ...
        while (queue.length > 0) {
            ...
            const neighbors = (adjList.get(current) ?? []).sort((a, b) =>
                a.localeCompare(b)
            );
