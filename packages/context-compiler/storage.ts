...
export class SnapshotStorage {
    ...
    async latestReference(): Promise<SnapshotReference | null> {
        const index = await this.loadIndex();
    ...
    async compact(keepCount: number = 10): Promise<number> {
        const index = await this.loadIndex();
        if (index.length <= keepCount) return 0;
    ...
    private async loadIndex(): Promise<SnapshotReference[]> {
        try {
            const raw = await fs.readFile(this.indexPath, "utf8");
            return JSON.parse(raw) as SnapshotReference[];
        } catch {
            return [];
        }
    }

    private async saveIndex(index: SnapshotReference[]): Promise<void> {
        try {
