...
export class DependencyAnalyzer {
    ...
    analyze(context: SnapshotContext): SnapshotDependency[] {
        ...
        if (context.indexData) {
            const extracted = this.extractFromIndex(
                context.indexData,
                context.workspaceRoot
            );
            deps.push(...extracted);
    ...
    private extractFromRelationships(data: any, workspaceRoot: string): SnapshotDependency[] {
        const deps: SnapshotDependency[] = [];

        if (!data || typeof data !== "object") return deps;
        ...
        for (const rel of data) {
            ...
            const importNames: string[] = rel.names || rel.importNames || [];
    ...
    private extractFromIndex(data: any, workspaceRoot: string): SnapshotDependency[] {
        const deps: SnapshotDependency[] = [];
        ...
        for (const item of items) {
            ...
            const imports: any[] = item.imports || item.dependencies || [];
    ...
    private normalizePath(p: string, base: string): string {
    ...
    private resolveKind(
        raw: string
    ): SnapshotDependency["kind"] {
        const lower = (raw || "").toLowerCase();
        if (lower.includes("export")) return "export";
        if (lower.includes("dynamic")) return "dynamic";
        if (lower.includes("re-export") || lower.includes("reexport")) return "re-export";
        return "import";
    }
}
