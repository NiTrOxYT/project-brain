...
export class SnapshotCollector {
    ...
    private async loadEvolution(): Promise<any> {
        const evoPath = path.join(
            this.workspaceRoot, "index", "evolution", "analytics.json"
        );
        const data = await this.loadJsonSafe(evoPath);
        return data ?? { fileHistory: [], coChangeRelationships: [] };
    }

    private async loadLearning(): Promise<any> {
    ...
    private async loadJsonSafe(filePath: string): Promise<any> {
        try {
