...
export class SnapshotValidator {
    ...
    validate(snapshot: SemanticSnapshot): SnapshotValidationResult {
        const errors: string[] = [];
        ...
        for (const section of snapshot.sections) {
            ...
            if (section.estimatedTokens !== expectedTokens) {
                warnings.push(
                    `Section '${section.id}' token estimate mismatch: stored=${section.estimatedTokens}, computed=${expectedTokens}.`
                );
            }
        }
        ...
        const computedTotal = snapshot.sections.reduce(
            (acc, s) => acc + Math.ceil(s.content.length / 4),
            0
        );
        if (snapshot.metadata.estimatedTokens !== computedTotal) {
            warnings.push(
                `Snapshot total token estimate mismatch: stored=${snapshot.metadata.estimatedTokens}, computed=${computedTotal}.`
            );
