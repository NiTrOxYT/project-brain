...
import type { LaunchReport }           from "./types.js";
...
import { KernelContext, GlobalPaths }   from "../kernel/index.js";
import type {
    GatewaySession,
    GatewayEvent,
    TimelineEntry,
...
const EVENT_LABELS: Partial<Record<GatewayEventKind, string>> = {
...
export class AiGatewayService {
    ...
    private readonly globalPaths:    GlobalPaths;

    private readonly optimizer: GatewayPromptOptimizer;
    private readonly sessions:  GatewaySessionStore;
    ...
    async run(
        ...
        const unsubscribeTimeline = this.bus.on("*", (ev: GatewayEvent) => {
            ...
            if (ev.payload["durationMs"] != null) {
                entry.durationMs = ev.payload["durationMs"] as number;
            }
            if (ev.payload["detail"] != null) {
                entry.detail = ev.payload["detail"] as string;
                ...
                const patterns = ev.payload["patterns"] as number | undefined;
                if (patterns != null) {
        ...
        try {
            ...
            if (!resolution.executableExists || !resolution.executable) {
                ...
                throw new Error(
                    `Failed to launch provider "${providerId}"\n` +
                    `Resolved Binary\n` +
            ...
            try { resolution = await resolver.resolve(providerId); } catch {}
            ...
            (session as any).launchReport = report;
