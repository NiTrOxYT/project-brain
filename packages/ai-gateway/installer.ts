...
import type { GlobalConfig, ProviderRegistration } from "./types.js";
...
export interface InstallResult {
    discovered: { id: string; binaryPath: string; version: string }[];
    generated:  { id: string; wrapperPath: string }[];
    warnings:   string[];
    uninstalled: string[];
}
...
export class GatewayInstaller {
     ...
     */
    async install(opts: InstallerOptions = {}): Promise<InstallResult> {
        ...
        if (!this.paths.isBinInPath()) {
            result.warnings.push(
                `Project Brain bin directory "${binDir}" is not in your PATH. ` +
    ...
    private loadConfig(): GlobalConfig {
        ...
        try {
            return JSON.parse(fs.readFileSync(this.paths.configPath, "utf8")) as GlobalConfig;
        } catch {
            return {
                version:            "1",
                installedProviders: {},
    ...
    private generateWrapperContent(providerId: string): string {
        return `#!/bin/sh
...
`;
    ...
    private async handleUninstall(dryRun: boolean, result: InstallResult): Promise<void> {
