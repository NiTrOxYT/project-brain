...
import fs from "fs";
import type { AggregateMetrics, ProviderStats, GatewaySession } from "./types.js";
...
export class GatewayMetricsStore {
    ...
    load(): AggregateMetrics {
        ...
        try {
            ...
            return JSON.parse(raw) as AggregateMetrics;
        } catch (err) {
            throw new MetricsStoreError(
                `Failed to load aggregate metrics: ${(err as Error).message}`
            );
        }
    }
     ...
     */
    update(session: GatewaySession, learningPatternsAdded = 0): void {
        try {
            const current  = this.load();
            const updated  = this.applySession(current, session, learningPatternsAdded);
            this.persist(updated);
        } catch (err) {
            if (err instanceof MetricsStoreError) throw err;
            throw new MetricsStoreError(
                `Failed to update metrics: ${(err as Error).message}`
            );
        }
    }
    ...
    private applySession(
        ...
        learningPatternsAdded: number
    ): AggregateMetrics {
        const m = session.metrics;
        const n = current.totalSessions;

        // Running-average helper
        const runAvg = (oldAvg: number, newVal: number, newN: number): number =>
            newN === 1 ? newVal : (oldAvg * (newN - 1) + newVal) / newN;

        const newN  = n + 1;
        const newTotal: AggregateMetrics = {
            totalSessions:       newN,
    ...
    private estimateCostSaved(promptTokens: number, optimizedTokens: number): number {
    ...
    private computeSessionDuration(
        ...
        if (!session.completedAt) return current.avgSessionDuration;
    ...
    private updateProviderStats(
        ...
        const runAvg = (oldAvg: number, newVal: number, n: number): number =>
            n === 1 ? newVal : (oldAvg * (n - 1) + newVal) / n;
    ...
    private persist(metrics: AggregateMetrics): void {
        fs.mkdirSync(this.paths.metricsDir, { recursive: true });
        fs.writeFileSync(
            this.paths.aggregateMetricsPath,
     ...
     */
    savedTokens(metrics?: AggregateMetrics): number {
