...
import { ContextPackage, ContextFile, ContextSymbol, ContextRelationship, ContextGraph } from "./types.js";
...
export class ContextAssemblerService {
    ...
    private readonly budgeter: ContextBudgetService;
    ...
    async assemble(
        ...
        try {
                ...
                if (cand.type === "file") {
                    ...
                    try {
                        content = await fs.readFile(
                            path.join(this.projectRoot, cand.id),
                            "utf8"
                        );
                    } catch {
                        // ignore
            ...
            const selectedMemoryEntriesList: any[] = [];
            ...
            const filteredSymbols: ContextSymbol[] = [];
            ...
            const relationshipsPath = path.join(this.workspaceRoot, "index", "relationships.json");
            const filteredRelationships: ContextRelationship[] = [];
            const seenRels = new Set<string>();

            if (options?.includeRelationships !== false && await this.filesystem.exists(relationshipsPath)) {

                const relData = await this.filesystem.readJson<{ relationships: any[] }>(relationshipsPath);
            ...
            const graphPath = path.join(this.workspaceRoot, "graph", "graph.json");
            let filteredGraph: ContextGraph = { nodes: [], edges: [] };

            if (options?.includeGraph !== false && await this.filesystem.exists(graphPath)) {

                const graphData = await this.filesystem.readJson<{ nodes: any[]; edges: any[] }>(graphPath);
            ...
            let filteredExecutionGraph: ContextGraph | undefined;

            if (options?.includeExecution === true && await this.filesystem.exists(execPath)) {

                const execData = await this.filesystem.readJson<{ nodes: any[]; edges: any[] }>(execPath);
            ...
            const evolutionAnalytics = evolutionExists
                ? await this.filesystem.readJson<{ fileHistory: any[] }>(evolutionPath)
                : { fileHistory: [] };
