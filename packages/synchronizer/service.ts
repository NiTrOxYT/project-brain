...
export class SynchronizerService {
    ...
    async synchronize(): Promise<SynchronizationState> {
        ...
        try {
            ...
            const existingIndex = await this.filesystem.readJson<IndexResult>(indexPath);
            ...
            let symbolsList: SymbolRecord[] = [];

            if (await this.filesystem.exists(symbolsPath)) {
                const symbolsData = await this.filesystem.readJson<SymbolIndex>(symbolsPath);
            ...
            let importsList: ImportRecord[] = [];

            if (await this.filesystem.exists(importsPath)) {
                const importsData = await this.filesystem.readJson<ImportIndex>(importsPath);
            ...
            let relationshipsList: RelationshipRecord[] = [];

            if (await this.filesystem.exists(relationshipsPath)) {
                const relData = await this.filesystem.readJson<RelationshipIndex>(relationshipsPath);
            ...
            if (await this.filesystem.exists(semanticPath)) {
                const semanticData = await this.filesystem.readJson<{ entries: any[] }>(semanticPath);
    ...
    private async scanProject(): Promise<Map<string, IndexedFile>> {
