...
async function runSuite() {
    ...
    await test("Test 4: Verify prompt assembler ranks sections correctly", async () => {
        ...
        const systemSecIdx = sections.findIndex((s: any) => s.id === "system");
        const valSecIdx = sections.findIndex((s: any) => s.id === "validation");
    ...
    await test("Test 5: Optimizer removes empty sections & normalizes spacing", () => {
        ...
        assert.ok(optimizations.some((o: any) => o.type === "whitespace-normalization"));
        assert.ok(optimizations.some((o: any) => o.type === "dead-code-pruning"));
    });
    ...
    await test("Test 6: Optimizer compresses verbose sections", () => {
        ...
        assert.ok(optimizations.some((o: any) => o.type === "summary-compression"));
    });
    ...
    await test("Test 8: Budgeter respects provider limits and truncates low-priority", () => {
        ...
        const tightProfile = {
            ...
            contextWindow: 50,
            metadata: {} as any,
            limits: { maxContextTokens: 50 } as any,
            pricing: {} as any
        } as any;

        const { budgetedSections: budgeted } = (service as any).budgeter.budget(rawSections, tightProfile);
        assert.ok(budgeted.find((s: any) => s.id === "system"));
        assert.ok(budgeted.find((s: any) => s.id === "files").content.includes("Truncated"));
    });
    ...
    await test("Test 11: Validator throws error on invalid task type capability", () => {
        ...
        assert.throws(() => {
            service.validate(
                { id: "test", type: "validate", title: "test", status: "Pending", prerequisites: [] },
                sections,
                profile,
                { actualTokens: 100 } as any
            );
    ...
    await test("Test 12: Validator throws error on massive token overflow", () => {
        ...
        assert.throws(() => {
            service.validate(
                { id: "test", type: "create", title: "test", status: "Pending", prerequisites: [] },
                massiveSections,
                profile,
                { actualTokens: 900_000 } as any
            );
        }, /exceeds/);
    });
    ...
    await test("Test 15: Metrics recording and retrieval", async () => {
        const getCompilations = (s: any) => Object.values(s.providerUtilization).reduce((a: number, b: any) => a + b, 0);
    ...
    await test("Test 17: Mock provider executes with ProviderExecutionRequest payload", async () => {
        ...
        const request = await service.buildExecutionRequest(
            { id: "mock-t", type: "create", title: "mock task", status: "Pending", prerequisites: [] },
            { workspaceRoot: WORKSPACE_DIR },
            provider.id,
            provider.profile()
        );
    ...
    await test("Test 20: Budgeter guarantees system rules are never truncated", () => {
        ...
        const tightProfile = {
            ...
            contextWindow: 10,
            metadata: {} as any,
            limits: { maxContextTokens: 10 } as any, // extremely tiny budget
            pricing: {} as any
        } as any;
        const { budgetedSections } = (service as any).budgeter.budget(rawSections, tightProfile);
        const sys = budgetedSections.find((s: any) => s.id === "system");
