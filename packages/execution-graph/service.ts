...
export class ExecutionGraphService {
    ...
    async build(): Promise<ExecutionGraph> {
        ...
        try {
            ...
            const transitiveImports = new Map<string, Set<string>>();
            ...
            const simpleNameMap = new Map<string, SymbolInfo[]>();

            const registerSymbol = (qualifiedName: string, file: string, kind: string) => {
                const info: SymbolInfo = { qualifiedName, file, kind };
            ...
            for (const filePath of tsFiles) {
                ...
                const visitDiscovery = (node: ts.Node) => {
                    let pushed = false;
                    let kind = "";
                    let name = "";
            ...
            const resolveTarget = (targetName: string, sourceFile: string): SymbolInfo | null => {
            ...
            for (const filePath of tsFiles) {
                ...
                const visitAnalysis = (node: ts.Node) => {
                    let pushed = false;
                    let kind = "";
                    let name = "";
                    ...
                    const addEdge = (targetName: string, type: ExecutionEdgeType) => {
                    ...
                    if (ts.isNewExpression(node)) {
                        ...
                        if (ts.isCallExpression(inner) || ts.isNewExpression(inner)) {
                            const callee = (inner as any).expression;
                            if (callee) {
            ...
            await this.filesystem.writeJson(
                path.join(graphDirPath, "execution-graph.json"),
                graph
            );
    ...
    async buildIncremental(
        ...
        try {
            ...
            const existingGraph = await this.filesystem.readJson<ExecutionGraph>(graphPath);
