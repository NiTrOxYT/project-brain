...
export class PromptAssembler {
    assemble(context: PromptContext, profile: PromptProviderProfile): PromptSection[] {
        ...
        if (archEntries.length > 0) {
            const archLines = archEntries.map((e: any) => `- [${e.category}] ${e.title}: ${e.description}`).join("\n");
            sections.push({
                id: "architecture",
        ...
        if (fused.length > 0) {
            const fileLines = fused.map((c: any) => `File Path: ${c.path} (Relevance Score: ${c.score})`).join("\n");
            sections.push({
                id: "files",
                name: "Relevant Files",
                content: fileLines,
        ...
        if (planNodes.length > 0) {
            const planLines = planNodes.map((n: any) => `- Task ${n.id} [${n.type}]: ${n.title} (Status: ${n.status})`).join("\n");
            sections.push({
                id: "execution-graph",
        ...
        if (optimizations.length > 0) {
            const optLines = optimizations.map((o: any) => `- Rule [${o.ruleType}]: ${o.id} (Confidence: ${o.confidence})`).join("\n");
            sections.push({
                id: "learning",
