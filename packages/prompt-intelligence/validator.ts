...
export class PromptValidator {
    validate(
        ...
        // 3. Missing files
        if (task.file) {
            const hasFileContent = sections.some(
                s => s.id.includes("file") && s.content.includes(task.file)
            );
        ...
        if (budget.actualTokens > profile.contextWindow) {
            throw new PromptBudgetError(
                `Prompt validation failed: Token overflow. Actual tokens ${budget.actualTokens} exceeds provider window limit ${profile.contextWindow}.`
            );
        }
