...
export class BrainInstaller {
    ...
    async install(opts: InstallerRunOptions = {}): Promise<InstallerResult> {
        ...
        try {
            ...
            const plugins  = (this.ctx ? this.ctx.plugins.list("provider") : AdapterRegistry.list()) as any[];
            const targets  = opts.providerId
                ? plugins.filter(a => a.id === opts.providerId)
                : plugins;
            ...
            for (const adapter of targets) {
                try {
                    ...
                    // Provider binary not found — skip silently
                }
            }
