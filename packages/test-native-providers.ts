...
function writeMockBinary(tempDir: string, providerName: string, binName: string, behavior: string): string {
...
interface ProviderTestConfig {
    ...
    envVar: string;
    binName: string;
    ctor: new (execService?: ProviderExecutionService) => any;
    supportsStreaming: boolean;
    expectedModel: string;
}

const PROVIDERS_TO_TEST: ProviderTestConfig[] = [
    {
        id: "codex",
        name: "Codex",
        envVar: "CODEX_BIN",
...
async function runTestsForProvider(cfg: ProviderTestConfig): Promise<void> {
    console.log(`\n===============================================================`);
    ...
    try {
        ...
        const applicable = response.artifacts.map((art: any) => ({
            id: art.id,
            taskId: task.id,
