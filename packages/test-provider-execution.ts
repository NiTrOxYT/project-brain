...
import { StreamProcessor, ChannelProcessor } from "./provider-execution/stream.js";
import { TimeoutManager } from "./provider-execution/timeout.js";
...
import { DiagnosticsCollector } from "./provider-execution/diagnostics.js";
import {
    ExecutionRequest,
    ExecutionMetrics,
    StreamChunk,
...
function req(
    id: string,
    args: string[],
    extra: Partial<ExecutionRequest> = {}
): ExecutionRequest {
...
async function test01_ProcessStarts(): Promise<void> {
    console.log("\n── 01. Process starts ────────────────────────────────────────");
...
async function test02_ProcessExitsSuccessfully(): Promise<void> {
...
async function test03_StdoutCaptured(): Promise<void> {
    console.log("\n── 03. stdout captured ───────────────────────────────────────");
    const runner = new ProcessRunner();
    const result = await runner.run(
        req("t03", nodeScript(`process.stdout.write("hello stdout\\n"); process.exit(0);`))
    );
...
async function test04_StderrCaptured(): Promise<void> {
    console.log("\n── 04. stderr captured ───────────────────────────────────────");
    const runner = new ProcessRunner();
    const result = await runner.run(
        req("t04", nodeScript(`process.stderr.write("hello stderr\\n"); process.exit(0);`))
    );
...
async function test05_StdinWorks(): Promise<void> {
    console.log("\n── 05. stdin works ───────────────────────────────────────────");
    const runner = new ProcessRunner();
    const script = `
        let data = "";
    ...
    `;
    const result = await runner.run(
        req("t05", nodeScript(script), { stdin: "hello-from-stdin" })
    );
    assert(result.output.stdout.includes("got:hello-from-stdin"), "stdin data received by process");
}

async function test06_EnvironmentVariables(): Promise<void> {
    console.log("\n── 06. Environment variables ─────────────────────────────────");
    const runner = new ProcessRunner();
    const result = await runner.run(
        req("t06", nodeScript(`process.stdout.write(process.env.MY_VAR || ""); process.exit(0);`), {
...
async function test07_WorkingDirectory(): Promise<void> {
    console.log("\n── 07. Working directory ─────────────────────────────────────");
    const dir = tempDir();
    try {
        const runner = new ProcessRunner();
        const result = await runner.run(
            req("t07", nodeScript(`process.stdout.write(process.cwd()); process.exit(0);`), { cwd: dir })
        );
...
async function test08_Cancellation(): Promise<void> {
    console.log("\n── 08. Cancellation ──────────────────────────────────────────");
    const svc = new ProviderExecutionService();
    ...
    assert(
        result instanceof ProcessCancelledError || result?.state === "Cancelled" || result?.exitCode !== 0,
        "Process cancelled or stopped early"
    );
    await svc.shutdown();
}

async function test09_GracefulShutdown(): Promise<void> {
...
async function test10_StartupTimeout(): Promise<void> {
    console.log("\n── 10. Startup timeout ───────────────────────────────────────");
...
async function test11_IdleTimeout(): Promise<void> {
    console.log("\n── 11. Idle timeout ──────────────────────────────────────────");
    ...
    if (caught instanceof ProcessTimeoutError) {
        assert(
            caught.timeoutKind === "idle",
            `Kind = idle (got ${caught.timeoutKind})`
        );
    }
}

async function test12_ExecutionTimeout(): Promise<void> {
...
async function test13_RetryPolicy(): Promise<void> {
...
async function test14_TransientRetry(): Promise<void> {
...
async function test15_PermanentFailure(): Promise<void> {
...
async function test16_Diagnostics(): Promise<void> {
    console.log("\n── 16. Diagnostics ───────────────────────────────────────────");
    const svc = new ProviderExecutionService();
...
async function test17_ResourceCleanup(): Promise<void> {
    console.log("\n── 17. Resource cleanup ──────────────────────────────────────");
    const svc = new ProviderExecutionService();
...
async function test18_SandboxCreation(): Promise<void> {
    console.log("\n── 18. Sandbox creation ──────────────────────────────────────");
...
async function test19_SandboxCleanup(): Promise<void> {
...
async function test20_DeterministicExecution(): Promise<void> {
...
async function test21_ParallelProcessExecution(): Promise<void> {
    ...
    const results = await Promise.all(
        Array.from({ length: 4 }, (_, i) =>
            runner.run(req(`t21-${i}`, nodeScript(`
                setTimeout(() => {
...
async function test22_StreamOrdering(): Promise<void> {
...
async function test23_LargeStdout(): Promise<void> {
...
async function test24_LargeStderr(): Promise<void> {
...
async function test25_ProcessCrashHandling(): Promise<void> {
    console.log("\n── 25. Process crash handling ────────────────────────────────");
    const runner = new ProcessRunner();
    const result = await runner.run(
        req("t25", nodeScript("process.exit(1)"), { retry: undefined })
    );
...
async function test26_InvalidExecutable(): Promise<void> {
    ...
    await assertRejects(
        ...
        ProcessSpawnError,
        "ProcessSpawnError on invalid executable"
    );
    await svc.shutdown();
}

async function test27_SignalForwarding(): Promise<void> {
    ...
    assert(
        err instanceof ProcessTimeoutError || err === undefined,
        "Process terminated within timeout window"
    );
}

async function test28_MetricsCorrectness(): Promise<void> {
...
async function test29_RepeatedExecutionDeterminism(): Promise<void> {
    ...
    const delays = Array.from({ length: 5 }, (_: unknown, attempt: number) => ev.computeDelay(attempt));
...
async function test30_BuildRegression(): Promise<void> {
