...
async function main() {
    ...
    assert(
        ...
        `Architecture entries > 0: ${snap.architecture.length} entries`
    );
    assert(
        !snap.architecture.every(e => e.title === "Legacy ADR entry"),
        "Architecture contains non-stub entries"
    );

    const archCategories = snap.architecture.map(e => e.category);
    assert(
        archCategories.includes("Language") || archCategories.includes("Structure") || archCategories.includes("Overview"),
        `Architecture has structural categories: [${archCategories.join(", ")}]`
    );
    ...
    assert(
        ...
        "Both instances return same snapshotId"
    );
    ...
    assert(
        typeof ctx.architectureSummary === "string" && ctx.architectureSummary.length > 20,
        `architectureSummary non-empty: "${ctx.architectureSummary.slice(0, 80)}..."`
    );
    assert(
        ctx.confidence > 0,
        `confidence > 0: ${ctx.confidence}`
    );
    assert(
        ctx.estimatedTokens > 0,
        `estimatedTokens > 0: ${ctx.estimatedTokens}`
    );
    assert(
        ctx.semanticMemory.length > 0,
        `semanticMemory.length > 0: ${ctx.semanticMemory.length}`
    );
    assert(
        ctx.rankedFiles.length > 0 || ctx.snippets.length > 0,
        `rankedFiles (${ctx.rankedFiles.length}) or snippets (${ctx.snippets.length}) > 0`
    );
    ...
    assert(
        !getCtxTool.inputSchema.required.includes("snapshotId"),
        "get_context: snapshotId not required"
    );
    ...
    assert(
        ctxResult && typeof ctxResult.architectureSummary === "string",
        "get_context: returns architectureSummary"
    );
    assert(
        ctxResult.confidence > 0,
        `get_context: confidence=${ctxResult.confidence}`
    );
    ...
    assert(
        typeof archResult.architectureSummary === "string" && archResult.architectureSummary.length > 10,
        `get_architecture: non-empty summary: "${archResult.architectureSummary.slice(0, 60)}..."`
    );
    assert(
        archResult.entries.length > 0,
        `get_architecture: entries > 0: ${archResult.entries.length}`
    );
    ...
    const mockLogger = {
        log: (msg: string) => { loggedLines.push(msg); },
    ...
    try {
        await runSearchMemory(
            { workspace: WORKSPACE, project: WORKSPACE, json: false, verbose: false, quiet: false },
            { query: "authentication", debug: true }
        );
    } finally {
    ...
    assert(
        loggedLines.some(l => l.includes("Loaded snapshot:")),
        "CLI search_memory: printed Loaded snapshot"
    );
    assert(
        loggedLines.some(l => l.includes("Memory entries:")),
        "CLI search_memory: printed Memory entries"
    );
    assert(
        loggedLines.some(l => l.includes("Returned entries:")),
        "CLI search_memory: printed Returned entries"
    );
    ...
    assert(
        searchRes.memories.length === 0,
        "CLI/MCP search_memory: unrelated query returns zero entries (no fallback mock)"
    );
