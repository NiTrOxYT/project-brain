...
class PriorityTestProvider implements AgentProvider {
    readonly id: string;
    ...
    constructor(id: string, name: string, priority: number, health: "Healthy" | "Degraded" | "Offline" = "Healthy", supportedRuntimeVersion = "1.0.0") {
        this.id = id;
        this.name = name;
    ...
    async execute(task: RuntimeTask, context: RuntimeContext, onEvent: (event: RuntimeEvent) => void): Promise<RuntimeResponse> {
...
async function main() {
    ...
    const task: RuntimeTask = { id: "NEGOTIATE-TASK-01", type: "custom-run", title: "Test selection", status: "Pending", prerequisites: [] };
    const negotiated = (runtime as any).registry.discover(task.type);

    console.log(`Negotiated selection order: ${negotiated.map((p: any) => `${p.id} (priority: ${p.priority})`).join(" -> ")}`);
    ...
    const negotiatedWithOffline = (runtime as any).registry.discover(task.type);
    console.log(`Selection order with offline: ${negotiatedWithOffline.map((p: any) => `${p.id} (health: ${p.health})`).join(" -> ")}`);
    
    // Offline provider (even with priority 100) must be skipped completely
    if (negotiatedWithOffline.some((p: any) => p.id === "offline-priority")) {
    ...
    const negotiatedWithIncompatible = (runtime as any).registry.discover(task.type);
    if (negotiatedWithIncompatible.some((p: any) => p.id === "incompatible-priority")) {
    ...
    const mwMutator: RuntimeMiddleware = {
        name: "mutator",
        async beforeExecute(req) {
    ...
    const timeoutTask: RuntimeTask = { id: "TIMEOUT-TASK", type: "custom-run", title: "Test timeouts", status: "Pending", prerequisites: [] };
