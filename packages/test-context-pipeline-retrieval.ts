...
async function runAll() {
    ...
    await fs.writeFile(
        path.join(memDir, "architecture.json"),
        JSON.stringify({
            entries: [
                {
                    category: "Core",
    ...
    await test("MCP brain.get_context", async () => {
        ...
        const containsMath = res.snippets.some((s: any) => s.code.includes("MathService"));
    ...
    await test("MCP brain.search_memory", async () => {
        ...
        const matchedSemantic = res.memories.some((m: any) => m.type === "semantic" && m.content.includes("logger.js"));
