...
function spawnBrain(cwd: string = "/tmp"): ChildProcess {
...
function collectStdio(child: ChildProcess, durationMs: number): Promise<{ stdout: string; stderr: string }> {
...
function sendLine(child: ChildProcess, obj: object): void {
...
async function testStdoutPurity(): Promise<void> {
    ...
    assert.strictEqual(nonJson.length, 0,
        `Non-JSON lines in stdout: ${JSON.stringify(nonJson)}`);
...
async function testInputSchemaInToolsList(): Promise<void> {
    const child = spawnBrain();
    setTimeout(() => sendLine(child, { jsonrpc: "2.0", method: "initialize",
    ...
    for (const tool of tools) {
        assert.ok(
            ...
            `Tool ${tool.name} missing inputSchema field`
        );
        assert.strictEqual(tool.inputSchema.type, "object",
            `Tool ${tool.name} inputSchema.type must be "object", got "${tool.inputSchema.type}"`);
        assert.ok(tool.inputSchema.properties !== undefined,
            `Tool ${tool.name} inputSchema.properties missing`);
    }
}

async function testAllRequiredToolsExposed(): Promise<void> {
    const child = spawnBrain();
    setTimeout(() => sendLine(child, { jsonrpc: "2.0", method: "initialize",
        params: { protocolVersion: "2024-11-05" }, id: 0 }), 100);
    setTimeout(() => sendLine(child, { jsonrpc: "2.0", method: "tools/list", params: {}, id: 1 }), 300);
...
async function testInitializeResponse(): Promise<void> {
    const child = spawnBrain();
    setTimeout(() => sendLine(child, { jsonrpc: "2.0", method: "initialize",
        params: { protocolVersion: "2024-11-05", capabilities: {} }, id: 1 }), 100);
...
async function testFullProtocolSequence(): Promise<void> {
    ...
    const initResp   = parsed.find((r: any) => r.id === 0);
    const listResp   = parsed.find((r: any) => r.id === 1);
    const callResp   = parsed.find((r: any) => r.id === 2);
