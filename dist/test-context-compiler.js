...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function buildMinimalSnapshot(overrides = {}) {
...
async function runSuite() {
    ...
    await test("Test 2: Fingerprint differs for different inputs", () => {
        const engine = new SnapshotFingerprintEngine();
        const ctx1 = { projectRoot: "/p", workspaceRoot: "/p/.brain", filePaths: ["a.ts"] };
        const ctx2 = { projectRoot: "/p", workspaceRoot: "/p/.brain", filePaths: ["b.ts"] };
    ...
    await test("Test 8: DependencyAnalyzer returns empty array for empty context", () => {
        const analyzer = new DependencyAnalyzer();
        const ctx = { projectRoot: TEST_WORKSPACE, workspaceRoot: TEST_WORKSPACE };
        const deps = analyzer.analyze(ctx);
    ...
    await test("Test 10: GraphCompiler creates file nodes from dependency edges", () => {
        ...
        assert.ok(graph.nodes.some(n => n.id === "file::src/a.ts"), "file node for a.ts must exist");
        assert.ok(graph.nodes.some(n => n.id === "file::src/b.ts"), "file node for b.ts must exist");
    });
    ...
    await test("Test 11: SnapshotBuilder sections have correct token estimates", () => {
        ...
        for (const section of snapshot.sections) {
            ...
            assert.strictEqual(section.estimatedTokens, expected, `Section '${section.id}' token estimate should be Math.ceil(chars/4)`);
        }
    });
    ...
    await test("Test 12: Snapshot sections are sorted by priority ascending", () => {
        ...
        for (let i = 1; i < snapshot.sections.length; i++) {
            assert.ok(snapshot.sections[i].priority >= snapshot.sections[i - 1].priority, "Sections must be in ascending priority order");
        }
    });
    ...
    await test("Test 14: Validator detects snapshotId / fingerprint hash mismatch", () => {
        ...
        assert.ok(result.errors.some(e => e.includes("snapshotId") || e.includes("fingerprint")), "Must report snapshotId/fingerprint mismatch");
    });
    ...
    await test("Test 15: Validator detects duplicate section IDs", () => {
        ...
        assert.ok(result.errors.some(e => e.toLowerCase().includes("duplicate")), "Must report duplicate section ID");
    });
    ...
    await test("Test 16: Optimizer deduplicates array sections", () => {
        ...
        assert.ok(Array.isArray(optimizedData) && optimizedData.length < data.length, "Optimizer should have removed duplicate array elements");
    });
    ...
    await test("Test 17: DeltaEngine correctly identifies added files", () => {
        ...
        assert.ok(result.changedFiles.some(f => f.path === "src/new.ts" && f.changeKind === "added"), "DeltaEngine must detect new file as added");
    });
    ...
    await test("Test 18: DeltaEngine correctly identifies removed files", () => {
        ...
        const result = delta.compute(s1, s2);
        assert.ok(result.changedFiles.some(f => f.path === "src/old.ts" && f.changeKind === "removed"), "DeltaEngine must detect removed file");
    });
    ...
    await test("Test 27: ContextCompilerService returns cache hit on second identical compile", async () => {
        ...
        assert.strictEqual(result1.snapshot.snapshotId, result2.snapshot.snapshotId, "Cache hit must return same snapshot");
    });
    ...
    await test("Test 29: ContextCompilerService.statistics() returns meaningful stats", async () => {
        ...
        assert.ok(stats.cacheHits + stats.cacheMisses === stats.totalCompilations, "cacheHits + cacheMisses must equal totalCompilations");
    });
