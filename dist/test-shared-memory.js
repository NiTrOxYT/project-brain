...
async function runSuite() {
    ...
    await test("Test 2: AgentRegistry — registration duplicate blocks", async () => {
        await assert.rejects(service.registerAgent({
            id: "claude-code",
            name: "Claude Code",
            capabilities: ["modify"],
            version: "1.0.0",
    ...
    await test("Test 3: AgentRegistry — unregister agent", async () => {
        ...
        const status = service.registry.status("claude-code");
    ...
    await test("Test 5: AgentRegistry — agent heartbeat updates session", async () => {
        ...
        const before = service.registry.status("gemini-cli").lastHeartbeatAt;
        await new Promise(r => setTimeout(r, 10));
        service.registry.heartbeat("gemini-cli");
        const after = service.registry.status("gemini-cli").lastHeartbeatAt;
        assert.ok(after.localeCompare(before) >= 0);
    });
    await test("Test 6: AgentRegistry — heartbeat on nonexistent throws", () => {
        assert.throws(() => service.registry.heartbeat("nonexistent"), AgentRegistrationError);
    });
    // ──────────────────────────────────────────────────────────────────────────
    // ASSIGNMENT ENGINE SCENARIOS (7-11)
    // ──────────────────────────────────────────────────────────────────────────
    await test("Test 7: AssignmentEngine — assigns based on capabilities", async () => {
        const tasks = [{ id: "t1", title: "Task 1", type: "validate", status: "Pending", prerequisites: [] }];
    ...
    await test("Test 8: AssignmentEngine — falls back to priority if no capability match", async () => {
        ...
        const tasks = [{ id: "t2", title: "Task 2", type: "unknown-capability", status: "Pending", prerequisites: [] }];
        const assigns = await service.assignTasks(tasks);
    ...
    await test("Test 9: AssignmentEngine — learning recommendation override", async () => {
        const tasks = [{ id: "t3", title: "Task 3", type: "validate", status: "Pending", prerequisites: [] }];
    ...
    await test("Test 10: AssignmentEngine — stable deterministic assignment fallback", async () => {
        ...
        const tasks = [{ id: "t4", title: "Task 4", type: "cleanup", status: "Pending", prerequisites: [] }];
        const assigns = await service.assignTasks(tasks);
    ...
    await test("Test 11: AssignmentEngine — throws on empty registry", async () => {
        ...
        await assert.rejects(emptyService.assignTasks([{ id: "t5", title: "Task 5", type: "cleanup", status: "Pending", prerequisites: [] }]), AssignmentError);
    });
    // ──────────────────────────────────────────────────────────────────────────
    ...
    await test("Test 14: Blackboard — publish warning", async () => {
        const w = await service.blackboard.publishWarning({
    ...
    await test("Test 15: Blackboard — publish fact", async () => {
        const fact = await service.blackboard.publishFact({
            key: "framework",
            value: "react",
            sourceAgentId: "aider"
        });
    ...
    await test("Test 16: Blackboard — publish issue", async () => {
        const issue = await service.blackboard.publishIssue({
            title: "Build failure",
    ...
    await test("Test 21: CoordinationEngine — claim task successfully", async () => {
        const task = { id: "claim-t", title: "Claim", type: "cleanup", status: "Pending", prerequisites: [] };
    ...
    await test("Test 22: CoordinationEngine — claim blocks on prerequisite", async () => {
        const task = { id: "claim-t2", title: "Claim 2", type: "cleanup", status: "Pending", prerequisites: ["claim-t"] };
        service.addTask(task);
    ...
    await test("Test 24: CoordinationEngine — waitBarrier matches completed tasks", async () => {
        const res = await service.coordination.waitBarrier(["claim-t"]);
        assert.strictEqual(res, true);
    });
    await test("Test 25: CoordinationEngine — waitBarrier fails on running task", async () => {
        const res = await service.coordination.waitBarrier(["claim-t2"]);
    ...
    await test("Test 31: ConflictResolver — resolution stores resolution record", () => {
        const state = service.model.getState();
    ...
    await test("Test 32: ConflictResolver — throws resolution error on invalid conflict state", async () => {
        const invalidRecord = { id: "invalid-c", conflictType: "file_collision", conflictingEntities: [], involvedAgents: [], timestamp: "", status: "open" };
        await assert.rejects(Promise.resolve().then(() => service.conflictResolver.resolve(invalidRecord)), ConflictError);
    });
    await test("Test 33: ConflictResolver — double resolve returns existing resolution", () => {
        const state = service.model.getState();
        const conf = state.conflicts.find((c) => c.status === "resolved");
        const res = service.conflictResolver.resolve(conf);
        assert.ok(res);
    });
    // ──────────────────────────────────────────────────────────────────────────
    ...
    await test("Test 41: Timeline — append persistence error logs check", async () => {
        const invalidTimeline = new service.timeline.constructor("/nonexistent-root-path/forbidden");
    ...
    await test("Test 45: SharedMemoryStorage — load non-existent returns null", async () => {
        const snap = await service.storage.loadSnapshot("nonexistent");
    ...
    await test("Test 46: WorkspaceEngine — blocks commit if open conflicts exist", async () => {
        ...
        const state = conflictsService.model.getState();
    ...
    await test("Test 48: ProviderRuntimeService — claims and completes tasks in Shared Memory", async () => {
        ...
        const mockProv = {
            ...
            capabilities: () => ["modify"],
            supportsCapability: (capability) => capability === "modify",
            health: async () => ({ status: "Healthy" }),
            execute: async (task, ctx, onEvent) => {
    ...
    await test("Test 49: LearningEngineService — records shared memory statistics in metadata", async () => {
        ...
        const res = await learning.learn([]);
        assert.ok(res.success);
        const metadata = await learning.storage.loadMetadata();
