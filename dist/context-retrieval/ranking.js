export class RetrievalRanker {
    rank(snapshot, candidatePaths, primaryTargets, symbols, learning) {
        const result = [];
        ...
        for (const p of candidatePaths) {
            ...
            const matchingLearning = learning.some((l) => Array.isArray(l.filesModified) && l.filesModified.some((fm) => fm.replace(/\\/g, "/").endsWith(normalized)));
            if (matchingLearning) {
                score += 15;
