...
export class GetContextTool {
    ...
    async execute(args) {
        try {
            ...
            process.stderr.write(`[brain.get_context] workspace=${normalizedWorkspace} snapshotId=${snapshotId} query="${args.query}"\n`);
            ...
            process.stderr.write(`[brain.get_context] result: confidence=${response.confidence} tokens=${response.estimatedTokens} ` +
