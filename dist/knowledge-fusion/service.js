...
export class KnowledgeFusionService {
    ...
    constructor(workspaceRoot, strategy = new WeightedFusionStrategy()) {
    ...
    async fuse(request) {
        try {
            ...
            const execRaw = new Map();
            const execFileRaw = new Map();
            ...
            const memoryEntriesMap = new Map();
            ...
            if (evolutionExists && evolutionAnalytics.fileHistory.length > 0) {
                ...
                const recencySignals = new Map();
                for (const file of evolutionAnalytics.fileHistory) {
                    let rec = 0.3;
                    if (file.recentlyChanged)
                        rec = 1.0;
                    else if (file.stableModule)
                        rec = 0.5;
                    else if (file.abandonedModule)
                        rec = 0.1;
                    recencySignals.set(file.path, rec);
                }
                const coChangeRawMap = new Map();
                for (const [semPath, semScore] of semanticSignals.entries()) {
            ...
            const addSignalValue = (id, type, source, val, metadata = {}) => {
            ...
            for (const c of finalCandidates) {
                ...
                if (c.provenance.includes("execution"))
                    executionContribution++;
                if (c.provenance.includes("relationships"))
                    relationshipContribution++;
                if (c.provenance.includes("graph"))
                    graphContribution++;
                if (c.provenance.includes("architecture"))
