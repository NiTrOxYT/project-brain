    ...
    async plan(request) {
        ...
        try {
            ...
            if (affectedFiles.length === 0) {
                try {
                    const indexData = await this.filesystem.readJson(path.join(this.workspaceRoot, "index", "index.json"));
            ...
            const addEdgeAcyclic = (from, to) => {
                // If it creates a cycle, skip
                const wouldCreateCycle = (src, target, pathSet) => {
                    if (src === target)
                        return true;
                    pathSet.add(src);
                    ...
                    if (currentDeps) {
                        for (const d of currentDeps) {
                            if (pathSet.has(d))
                                continue;
                            if (wouldCreateCycle(d, target, new Set(pathSet)))
                                return true;
                        }
                    }
                    return false;
            ...
            const computeDepth = (file, currentPath) => {
                let maxD = 0;
                const deps = dependencyMap.get(file);
                if (deps) {
                    for (const d of deps) {
                        if (currentPath.has(d))
                            continue;
                        currentPath.add(d);
            ...
            if (evolutionExists) {
                try {
                    const evoData = await this.filesystem.readJson(evolutionPath);
            ...
            try {
                ...
                for (const file of affectedFiles) {
                    ...
                    const matchesRule = memEntries.some((entry) => entry.title.toLowerCase().includes(filename) ||
                        entry.relatedFiles.some((f) => f.includes(file)));
                    if (matchesRule) {
            ...
            let complexityLabel = "Small";
            if (complexityScore >= 120)
                complexityLabel = "Very Large";
            else if (complexityScore >= 60)
                complexityLabel = "Large";
            ...
            const tasksList = [];
            const fileToAnalyzeTaskId = new Map();
            const fileToModifyTaskId = new Map();
            const fileToTestTaskId = new Map();
            const fileToValidateTaskId = new Map();
            ...
            for (const file of affectedFiles) {
                ...
                if (maxChurn > 10)
                    modifyTask.rationale.push("High historical churn file; handle with care");
                if (riskArchitecture > 0)
                    modifyTask.rationale.push("Matches architectural invariants recorded in memory");
            ...
            const graphNodes = tasksList.filter(t => !t.isRollback).map(t => t.id);
            const graphEdges = [];
