...
import { createKernelContext, runGatewaySession, queryGatewayHistory, findSessionById, runGatewayInstaller } from "./sdk/index.js";
...
class MockCertificationAdapter extends BaseProviderAdapter {
    id = "mock-cert";
    displayName = "Mock Certification Provider";
    version = "1.2.3";
    binaryName = "mock-provider";
    metadata() {
        return {
            id: this.id,
    ...
    async launch(opts) {
        ...
        let stdoutResolver;
        let waitResolver;
        const stdoutPromise = new Promise((res) => { stdoutResolver = res; });
        const waitPromise = new Promise((res) => { waitResolver = res; });
...
async function validateGate6And10() {
    ...
    let providerPid;
    ctx.eventBus.on("ProviderStarted", (ev) => {
