...
export class PromptBudgeter {
    ...
    budget(sections, profile) {
        ...
        const estimateTokens = (secs) => {
            const rendered = this.renderer.render(secs, profile);
            return Math.ceil(rendered.length / 4);
        };
        // Priority 1 to 10
        const getPriority = (sectionId) => {
            const id = sectionId.toLowerCase();
            if (id.includes("system"))
                return 1;
            if (id.includes("provider-rules") || id.includes("provider-rule"))
                return 2;
            if (id.includes("rule"))
                return 2;
            if (id.includes("task") || id.includes("instruction") || id.includes("template"))
                return 3;
            if (id.includes("constraint"))
                return 4;
            if (id.includes("arch"))
                return 5;
            if (id.includes("learn") || id.includes("history"))
                return 6;
            if (id.includes("file") || id.includes("symbol") || id.includes("code"))
                return 7;
    ...
    updateAllocation(allocated, sectionId, charLength) {
