...
import { ManifestManager, checksumContent } from "./manifest.js";
...
export class BrainInstaller {
        ...
        try {
            ...
            for (const res of targets) {
                ...
                if (res.resolvedBinary) {
                    const manifestStatus = this.manifest.verifyWrapper(res.providerId, INSTALLER_VERSION);
                    const status = manifestStatus === "ok" ? "current" :
            ...
            for (const existing of manifestProviders) {
                if (!discoveredIds.has(existing)) {
                    result.removed.push(this.handleRemovedProvider(existing, dryRun, journal));
                }
            }
    ...
    // ─── Wrapper Generation ──────────────────────────────────────────────────
    async generateWrapper(disc, dryRun, repair, journal) {
    ...
    generateWrapperContent(providerId, realBinaryPath) {
        return `#!/bin/sh
# ──────────────────────────────────────────────────────────────────────────────
...
`;
    ...
    handleRemovedProvider(providerId, dryRun, journal) {
        if (dryRun)
            return { id: providerId, action: "archived" };
    ...
    // ─── Uninstall ───────────────────────────────────────────────────────────
    async handleUninstall(dryRun, result, journal) {
    ...
    runDiagnostics(result) {
        ...
        for (const gen of result.generated) {
            if (gen.action === "skipped" && !fs.existsSync(gen.wrapperPath))
                continue;
     ...
     */
    validateWrapperExecution(providerId) {
        const binPath = this.paths.binEntry(providerId);
        try {
            if (!fs.existsSync(binPath)) {
