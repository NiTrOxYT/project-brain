...
export async function runSharedMemory(opts, sub, cmdOpts) {
    ...
    try {
        switch (sub) {
            ...
            case "agents": {
                const model = svc.model ?? {};
                const agentMap = model.agents ?? new Map();
                ...
                if (opts.json) {
                    ...
                    if (agents.length === 0) {
                        ...
                        ], agents.map((a) => ({ id: a.id ?? "", name: a.name ?? "", phase: a.phase ?? "" }))));
                    }
                }
                break;
            }
            ...
            case "snapshot": {
                const snapshotId = cmdOpts["snapshot-id"]
                    ?? `snap-${Date.now()}`;
