...
export class ContextCompilerService {
    ...
    cache;
    storage;
    deltaEngine = new SnapshotDeltaEngine();
    optimizer = new SnapshotOptimizer();
    ...
    async compile(req = {
        ...
        try {
            ...
            // Stage 2: Fingerprint
            const fingerprint = await this.runStage(stages, "Fingerprint", () => Promise.resolve(this.fpEngine.compute(context)));
            // Cache hit?
            if (!req.force) {
            ...
            const files = await this.runStage(stages, "Normalizer:Files", () => Promise.resolve(this.builder.extractFiles(context)));
            const symbols = await this.runStage(stages, "Normalizer:Symbols", () => Promise.resolve(this.builder.extractSymbols(context)));
            // Stage 4: Dependency analysis
            const dependencies = await this.runStage(stages, "DependencyAnalyzer", () => Promise.resolve(this.depAnalyzer.analyze(context)));
            // Stage 5: Relationships
            const relationships = await this.runStage(stages, "Relationships", () => Promise.resolve(this.builder.extractRelationships(context)));
            // Stage 6: Architecture
            const architecture = await this.runStage(stages, "Architecture", () => Promise.resolve(this.builder.extractArchitecture(context)));
            // Stage 7: Evolution
            const evolution = await this.runStage(stages, "Evolution", () => Promise.resolve(this.builder.extractEvolution(context)));
            // Stage 8: Learning
            const learning = await this.runStage(stages, "Learning", () => Promise.resolve(this.builder.extractLearning(context)));
            // Stage 9: Graph Compilation
            const graph = await this.runStage(stages, "GraphCompiler", () => Promise.resolve(this.graphCompiler.compile(context, dependencies)));
            ...
            const snapshot = await this.runStage(stages, "SnapshotBuilder", () => Promise.resolve(this.builder.build({
                context,
                fingerprint,
                files,
                symbols,
            ...
            // Stage 11: Optimize
            const optimizerResult = await this.runStage(stages, "Optimizer", () => Promise.resolve(this.optimizer.optimize(snapshot)));
            ...
            const optimizedSnapshot = {
                ...
                metadata: {
                    ...snapshot.metadata,
                    estimatedTokens: optimizerResult.sections.reduce((acc, s) => acc + s.estimatedTokens, 0),
                    stageCount: stages.length
                }
            };
            ...
            // Stage 12: Validate
            const validation = await this.runStage(stages, "Validator", () => Promise.resolve(this.validator.validate(optimizedSnapshot)));
            // Stage 13: Store
            ...
            throw new SnapshotCompilationError(`Compilation failed: ${err.message}`);
        }
    }
    /**
     ...
     */
    async compileIncremental(changedFilePaths) {
        const latestRef = await this.storage.latestReference();
        return this.compile({
            projectRoot: this.projectRoot,
            workspaceRoot: this.workspaceRoot,
     ...
     */
    compileSection(sectionId, name, kind, priority, data, sourcePaths) {
     ...
     */
    async compileChangedFiles(dirtyFiles) {
     ...
     */
    async compileDirtySymbols(dirtyFiles) {
        const context = await this.collector.collect();
     ...
     */
    async compileDirtyDeps(dirtyFiles) {
        const context = await this.collector.collect();
     ...
     */
    async compileDirtyRels(dirtyFiles) {
        const context = await this.collector.collect();
     ...
     */
    async compileDirtyGraph(dirtyFiles) {
        const context = await this.collector.collect();
     ...
     */
    async compilePatch(prevSnapshotId, patch) {
