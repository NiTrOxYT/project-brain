...
// ─── Imports ─────────────────────────────────────────────────────────────────
import { ZshShellProvider, BashShellProvider, FishShellProvider, PowerShellShellProvider, CmdShellProvider, NushellShellProvider, detectPlatform, detectShellProvider, } from "./installer/shell-provider.js";
import { PathManager } from "./installer/path-manager.js";
import { ManifestManager, checksumContent, } from "./installer/manifest.js";
import { BrainInstaller, INSTALLER_VERSION, EXIT_SUCCESS, EXIT_FATAL, EXIT_SHELL_CONFIG_DENIED, EXIT_PROVIDER_DISCOVERY, EXIT_WRAPPER_VALIDATION, } from "./installer/installer.js";
...
function test(name, fn) {
    return Promise.resolve(fn()).then(() => { testsPassed++; console.log(`  ✓  ${name}`); }, (err) => { testsFailed++; console.log(`  ✗  ${name}`); console.log(`     ${err.message}`); });
}
function tmpDir() {
...
async function run() {
    ...
    await test("ZshShellProvider adds PATH entry to temp .zshrc", () => {
        ...
        const sp = new ZshShellProvider();
        sp.configFileName = path.relative(os.homedir(), configFile);
    ...
    await test("isInConfig detects $HOME equivalent", () => {
        ...
        const sp = new ZshShellProvider();
        const origConfig = sp.configFileName;
    ...
    await test("BrainInstaller lock prevents concurrent runs", async () => {
        ...
        assert(result.warnings.some(w => w.includes("Another brain install")), "Should warn about concurrent install");
        cleanup(dir);
    });
    await test("BrainInstaller stale lock is cleaned", async () => {
        ...
        assert(!result.warnings.some(w => w.includes("Another brain install")), "Stale lock should be cleaned up");
        cleanup(dir);
    });
    ...
    await test("ProviderAdapter exposes binaryName", () => {
        ...
        for (const a of adapters) {
            assert(typeof a.binaryName === "string" && a.binaryName.length > 0, `${a.id} should have binaryName`);
        }
    });
    ...
    await test("Only one provider registry instance exists in runtime", async () => {
        ...
        const staticRegistry = AdapterRegistry.registry;
