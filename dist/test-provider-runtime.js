...
// ─── Tests ────────────────────────────────────────────────────────────────────
async function test01_ProviderRegistration() {
...
async function test02_ProviderOrdering() {
    ...
    assert(sorted.map(p => p.id).join(",") === sorted2.map(p => p.id).join(","), "Ordering is deterministic (repeated calls produce identical result)");
}
async function test03_CapabilityNegotiation() {
...
async function test04_ModelNegotiation() {
...
async function test05_HealthFiltering() {
    ...
    const healthReports = new Map([
        [mock.id, { status: "Healthy", authenticated: true, installed: true, latencyMs: 10, lastHeartbeat: new Date().toISOString(), version: "1.0.0" }],
        [claude.id, { status: "Offline", authenticated: false, installed: false, latencyMs: 0, lastHeartbeat: new Date().toISOString(), version: "1.0.0" }]
    ]);
...
async function test06_FallbackChain() {
    ...
    assert(result.selectedProvider === result2.selectedProvider &&
        result.fallbackChain.join(",") === result2.fallbackChain.join(","), "Fallback chain deterministic across repeated calls");
}
async function test07_ProviderPriorities() {
...
async function test08_SessionCreation() {
    console.log("\n── 08. Session Creation ──────────────────────────────────────");
...
async function test09_SessionReplay() {
...
async function test10_SessionPersistence() {
...
async function test11_StreamingEvents() {
    console.log("\n── 11. Streaming Events ──────────────────────────────────────");
...
async function test12_MiddlewareExecution() {
    ...
    const response = { taskId: "t1", status: "Completed", artifacts: [], metrics: { provider: mock.id, capability: "create", executionTime: 0, retries: 0, artifactsProduced: 0, eventsEmitted: 0, taskCount: 1, cancellationCount: 0, pauseCount: 0, resumeCount: 0 } };
    const metrics = { provider: mock.id, model: "mock-model-v1", taskId: "t1", promptTokens: 100, completionTokens: 200, latencyMs: 50, executionDurationMs: 50, estimatedCost: 0, retries: 0, workspaceWrites: 0, artifactsGenerated: 1, executionEvents: 1, streamEvents: 1, fallbackCount: 0, knowledgeCacheHits: 0, timestamp: new Date().toISOString() };
...
async function test13_RetryBehavior() {
    ...
    try {
        ...
        const flakyProvider = {
            ...
            capabilities: () => ["create"],
            health: async () => ({ status: "Healthy", authenticated: true, installed: true, latencyMs: 0, lastHeartbeat: new Date().toISOString(), version: "1.0.0" }),
            async execute(task, ctx, onEvent, onStream) {
                attempts++;
                if (attempts <= 2) {
        ...
        service.register(flakyProvider);
...
async function test14_PermanentFailureFallback() {
    ...
    try {
        ...
        service.register(permanentlyFailing);
...
async function test15_MetricsRecording() {
...
async function test16_CostAggregation() {
...
async function test17_MockProviderCompatibility() {
    ...
    assert(!mock.supportsCapability("nonexistent"), "Does not support unknown capability");
...
async function test18_ClaudeCodeAdapter() {
...
async function test19_CodexAdapter() {
    console.log("\n── 19. Codex Adapter ─────────────────────────────────────────");
...
async function test20_GeminiAdapter() {
...
async function test21_OllamaAdapter() {
    console.log("\n── 21. Ollama Adapter ────────────────────────────────────────");
...
async function test22_AiderAdapter() {
    console.log("\n── 22. Aider Adapter ─────────────────────────────────────────");
    const aider = new AiderProvider();
...
async function test23_OpenCodeAdapter() {
    console.log("\n── 23. OpenCode Adapter ──────────────────────────────────────");
...
async function test24_AgentRuntimeIntegration() {
...
async function test25_WorkspaceEngineCompatibility() {
    ...
    try {
        ...
        assert(!response.artifacts.some((a) => a.directWrite === true), "No provider bypassed workspace engine");
...
async function test26_OrchestratorIntegration() {
...
async function test27_QueryEngineDiagnostics() {
...
async function test28_DeterministicRepeatedExecution() {
    ...
    try {
        ...
        const ctx = { capability: "create", preferredModel: "claude-sonnet-4-5" };
        // Run negotiation 5 times — must be identical
        const results = Array.from({ length: 5 }, () => negotiator.negotiate(providers, ctx));
        const first = results[0];
        for (let i = 1; i < results.length; i++) {
            const r = results[i];
            assert(r.selectedProvider === first.selectedProvider, `Run ${i + 1}: selectedProvider identical (${r.selectedProvider})`);
            assert(r.selectedModel === first.selectedModel, `Run ${i + 1}: selectedModel identical (${r.selectedModel})`);
            assert(r.fallbackChain.join(",") === first.fallbackChain.join(","), `Run ${i + 1}: fallbackChain identical`);
            assert(r.capabilityScore === first.capabilityScore, `Run ${i + 1}: capabilityScore identical`);
        }
...
async function main() {
    ...
    const writeMock = (name, bin, exitOnList = false) => {
