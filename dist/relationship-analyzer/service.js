...
export class RelationshipAnalyzerService {
    ...
    async extractFromFile(relativePath, projectSymbols) {
        const fullPath = path.join(this.projectRoot, relativePath);
        const parsed = await this.parser.parse(fullPath);
        const output = [];
        this.extractRelationships(parsed.ast, relativePath, projectSymbols, output);
        return output;
    }
    async analyze() {
        try {
            ...
            const projectSymbols = new Set(symbolsData.symbols.map(sym => sym.name));
            ...
            for (const file of indexData.files) {
                ...
                const parsed = await this.parser.parse(fullPath);
                this.extractRelationships(parsed.ast, file.path, projectSymbols, relationships);
            }
            const result = {
                generatedAt: new Date().toISOString(),
                relationships
            };
            await this.filesystem.writeJson(path.join(this.workspaceRoot, "index", "relationships.json"), result);
            return result;
        }
        catch (error) {
    ...
    extractRelationships(sourceFile, relativePath, projectSymbols, relationships) {
        const scopeStack = [];
        const visit = (node) => {
            let pushed = false;
