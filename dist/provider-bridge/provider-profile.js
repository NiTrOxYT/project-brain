...
export class ProviderProfileRegistry {
    static generateProfile(providerId, workspaceRoot) {
        ...
        if (manifest.capabilities.supportsGlobalConfiguration)
            availableFeatures.push("global-configuration");
        if (manifest.capabilities.supportsWorkspaceConfiguration)
            availableFeatures.push("workspace-configuration");
        if (manifest.capabilities.supportsStdioMcp)
            availableFeatures.push("stdio-transport");
        if (manifest.capabilities.supportsHttpMcp)
            availableFeatures.push("http-transport");
        if (manifest.capabilities.supportsRuntimeToolDiscovery)
            availableFeatures.push("mcp-tool-discovery");
        if (manifest.capabilities.supportsRuntimeToolInvocation)
            availableFeatures.push("runtime-tool-invocation");
        if (manifest.capabilities.supportsBehaviorVerification)
            availableFeatures.push("behavioral-telemetry");
        // Strategy features
