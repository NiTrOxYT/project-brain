...
// ──────────────────────────────────────────────────────────────────────────────
export function mergeToml(content, key, values) {
...
export function unconfigureToml(content, key) {
    const lines = content.split(/\r?\n/);
    const targetHeader = `[mcp_servers.${key}]`;
    let startIdx = -1;
    let endIdx = -1;
...
export function mergeYamlMcpServers(content, entry) {
    const lines = content.split(/\r?\n/);
    let mcpServersIdx = -1;
    for (let i = 0; i < lines.length; i++) {
    ...
    if (entry.type)
        entryLines.push(`    type: ${entry.type}`);
    if (entry.command)
        entryLines.push(`    command: ${entry.command}`);
    if (entry.args) {
...
export function unconfigureYamlMcpServers(content, name) {
    const lines = content.split(/\r?\n/);
    let mcpServersIdx = -1;
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].trim().startsWith("mcpServers:")) {
...
export function mergeYamlAiderRead(content, filePath) {
    const lines = content.split(/\r?\n/);
    let readIdx = -1;
    for (let i = 0; i < lines.length; i++) {
...
export function unconfigureYamlAiderRead(content, filePath) {
    const lines = content.split(/\r?\n/);
    let readIdx = -1;
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].trim().startsWith("read:")) {
