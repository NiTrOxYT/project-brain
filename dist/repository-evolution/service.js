...
export class RepositoryEvolutionService {
    ...
    async initialize() {
        ...
        if (historyExists && analyticsExists) {
            try {
                const history = await this.filesystem.readJson(historyPath);
                if (history.repositoryHash === repositoryHash) {
                    const analytics = await this.filesystem.readJson(analyticsPath);
    ...
    async getAnalytics() {
        ...
        return this.filesystem.readJson(analyticsPath);
    }
    async getCurrentHeadHash() {
        try {
    ...
    async computeAnalytics(commits, repositoryHash) {
        ...
        for (const commit of sortedCommits) {
            ...
            for (const file of commit.files) {
                if (file.status === "D")
                    continue; // skip deleted files from active scoring
