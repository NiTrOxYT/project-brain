...
function searchPath(binaryName, excludeDir) {
    ...
    for (const entry of entries) {
        ...
        if (normalExclude && normalEntry === normalExclude)
            continue;
...
export class ProviderResolverService {
    ...
    // ─── Repair ──────────────────────────────────────────────────────────────
    async repair(providerId, realBinaryPath) {
        const adapter = AdapterRegistry.lookup(providerId);
...
`;
        ...
        // 3. Update manifest
        let manifest = { version: "1", wrappers: {}, updatedAt: new Date().toISOString() };
        if (fs.existsSync(manifestPath)) {
            try {
